...
- v5.2 遗留：Course 实体增加 CourseCategory（REQUIRED/ELECTIVE/PE），课程表单支持类别选择，学生首页展示总进度条和必修/选修/体育三条分类进度条（仅统计已发布且>=60分的成绩）。种子课程除羽毛球(体育)外均为必修。学分要求：必修18.5 + 选修6 + 体育1 = 总计25.5学分（Service 常量）。
